...
class BayesianOptimizer:
    ...
    def _fit_gp(self, X01: np.ndarray, y: np.ndarray) -> GaussianProcessRegressor:
        ...
        gp = GaussianProcessRegressor(
            kernel=kernel, normalize_y=False, n_restarts_optimizer=6, random_state=0
        )
        gp.fit(X01, y_norm)
        gp._y_mean_ = y.mean()
        gp._y_std_ = y_std
    ...
    def suggest(self, trials: list[TrialOut], n_candidates: int = 4000, rng_seed: int = 0):
